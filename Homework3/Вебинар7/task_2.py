...
class PersonInfo:
    def __init__(self, name: str, age: int, *pod):
        self.name = name
        self.age = age
        self.pod = pod

    def short_name(self):
        """Возвращает имя сотрудника в формате 'Фамилия И.'."""
        n1 = self.name.split(' ')
        return f'{n1[1]} {n1[0][0]}.'

    def path_deps(self):
        """Возвращает строку, в которой названия отделов разделены знаком '-->'."""
        n2 = ' --> '.join(self.pod)
        return n2

    def new_salary(self):
        """Возвращает размер заработной платы, рассчитанной по новой методике."""
        e_count = Counter(''.join(self.pod))
        sum_popular = sum([j[1] for j in e_count.most_common(3)])
        n3 = 1337 * self.age * sum_popular
        return n3
