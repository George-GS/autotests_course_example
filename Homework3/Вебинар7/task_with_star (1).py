...
class RomanNums:
    ...
    def from_roman(self):
        ...
        for index, letter in enumerate(list(self.rom_num)[:-1]):
            if roman_dict[letter] >= roman_dict[list(self.rom_num)[index + 1]]:
                arab_num += roman_dict[letter]
            else:
                arab_num -= roman_dict[letter]

        arab_num += roman_dict[self.rom_num[-1]]
        return arab_num

    def is_palindrome(self):
        """Проверяет, является ли римское число палиндромом"""
